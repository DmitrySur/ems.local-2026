...
use App\Models\User;
use App\Services\Incident\Reports\IncidentsReportsConfigEnum\DropsVoltageStrategy;
use App\Services\Incident\Reports\IncidentsReportsConfigEnum\InspectionsStrategy;
...
class CustomizableSummaryIncidentReportService
{
    protected ?PhpWord $wordDocument = null;

    protected ?Collection $collectionRecords = null;

    protected ?DatePeriodService $datePeriodService;

    protected TypeSummaryReportStrategy $typeSummaryReportStrategy;
    ...
    public function makeAndGetReportFilename(
        ?DatePeriodService        $datePeriodService = null,
        TypeSummaryReportStrategy $typeSummaryReportStrategy = TypeSummaryReportStrategy::IncidentsForPeriod,
    ): ?string
    {
        try {
            $this->typeSummaryReportStrategy = $typeSummaryReportStrategy;
            switch ($this->typeSummaryReportStrategy) {
                //Инциденты за период
                case TypeSummaryReportStrategy::IncidentsForPeriod:
                    $this->datePeriodService = $datePeriodService;
                    $this->collectionRecords = Collection::make(Incident::incidentsForCustomizableSummaryIncidentReportServiceByPeriod(
                        $this->datePeriodService->getStartDateTime(),
                        $this->datePeriodService->getEndDateTime(),
                    )->get())
                        ->groupBy('division.name');
                    break;
                // Список ННР инцидентов за период
                case TypeSummaryReportStrategy::NnrIncidentsForPeriod:
                    $this->datePeriodService = $datePeriodService;
                    $this->collectionRecords = Collection::make(Incident::incidentsForCustomizableSummaryIncidentReportServiceByPeriodNnr(
                        $this->datePeriodService->getStartDateTime(),
                        $this->datePeriodService->getEndDateTime(),
                    )->get())
                        ->groupBy('division.name');
                    break;
                //Инциденты в ремонте
                case TypeSummaryReportStrategy::IncidentsInRepair:
                    $this->collectionRecords = Collection::make(Incident::incidentsForCustomizableSummaryIncidentReportServiceInRepairAll()
                        ->get())
                        ->groupBy('division.name');
                    break;
                default:
                    throw new Exception('Неизвестный тип стратегии сводного отчета по инцидентам.');
            }

            $this->divisionsArray = Division::orderBy('report_position')
                ->pluck('name')
                ->toArray();
            if ($this->typeSummaryReportStrategy->getDropsVoltageStrategy() === DropsVoltageStrategy::WithDropsVoltage) {
                $this->dropVoltageCollectionRecords = DropVoltage::whereBetween(
                    'datetime_drop',
                    [
                        $this->datePeriodService->getStartDateTime()->toDateTimeString(),
                        $this->datePeriodService->getEndDateTime()->toDateTimeString(),
                    ]
                )
                    ->with([
            ...
            $filename = 'Отчет';
            if ($this->typeSummaryReportStrategy === TypeSummaryReportStrategy::IncidentsInRepair) {
                $filename .= ' журнал ремонта ' . now()->format('dmYHis');
            } else {
                $filename .= ' сводка ' .
                    Str::of($this->datePeriodService->getFormattedPeriodForReport())->replace(':', '-')->replace('период отчета ', '') .
                    ' ' . now()->format('dmYHis') .
                    Auth::id();
            }
            ...
            throw new Exception('Ошибка сервиса при создании сводного отчета по инцидентам: ' . $exception->getMessage());
        }
    }
    ...
    protected function makeMainContent(): void
    {
        ...
        switch ($this->typeSummaryReportStrategy) {
            ...
            // Список ННР инцидентов за период
            case TypeSummaryReportStrategy::NnrIncidentsForPeriod:
                $section->addText('Хроника инцидентов номерных ННР по Электромеханической службе Дирекции инфраструктуры');
                break;
            //Инциденты в ремонте
            case TypeSummaryReportStrategy::IncidentsInRepair:
                $section->addText('Хроника инцидентов в ремонте по Электромеханической службе Дирекции инфраструктуры');
                break;
            default:
                throw new Exception('Неизвестный тип стратегии сводного отчета по инцидентам.');
        }
        if (isset($this->datePeriodService)) {
            $section->addText($this->datePeriodService?->getFormattedPeriodForReport() ?? '');
        }
        ...
        foreach ($this->divisionsArray as $divisionItem) {
            ...
            $dropVoltagesRecordsByDivision = $this->dropVoltageCollectionRecords?->get($divisionItem) ?? null;
            if ($incidentRecordsByDivisionAndTypes || $dropVoltagesRecordsByDivision) {
                ...
                if ($this->typeSummaryReportStrategy->getDropsVoltageStrategy() === DropsVoltageStrategy::WithDropsVoltage &&
                    $dropVoltagesRecordsByDivision &&
                    count($dropVoltagesRecordsByDivision) > 0) {
                    $table->addRow();
                    $table->addCell(
                        Converter::cmToTwip(26.78),
                        [
    ...
    {
        if (isset($this->datePeriodService) && $this->datePeriodService->getStartDateTime() && $this->typeSummaryReportStrategy->getInspectionStrategy() === InspectionsStrategy::WithInspections) {
            try {
                $inspections = Inspection::where(function (Builder $query) {
                    ...
                    $query->where('date_start', $this->datePeriodService->getStartDateTime()->toDateString());
                })
                    ->orWhere(function (Builder $query) {
                        $query->where('type', InspectionsTypes::NightInspection);
                        $query->where('date_start', $this->datePeriodService->getEndDateTime()->toDateString());
                    })
                    ->with([
                        'division',
    ...
    protected function setFontStyleIncident(Incident $incident, ?int $fontSize = 12, bool $isUnderline = false): array
    {
        ...
        if (isset($this->datePeriodService)) {
            if (($incident->incident_classification === 'ННР' ||
                ...
                $incident->datetime_incident->between($this->datePeriodService->getStartDateTime(), $this->datePeriodService->getEndDateTime())
            ) {
    ...
    protected
    function setFontStyleDropVoltage(?int $fontSize = 10): array
    {
        return [
            'size' => $fontSize,
            'color' => '808080',
